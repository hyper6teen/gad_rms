...
<body>
	<div class="wrapper">
		...
		<div class="content">
			<div class="content-header">Department</div>
			...
			<div id='college-add-body' class="content-body">
				...
				function checkForm(dep, al, col)
				{
					...
					if (dep.length > 4 && al.length > 1 && col != "none") {
						
						
						$.ajax({
					   	type: 'post',
						url: 'lib.php',
					   	data: {functionName: 'addDepartment', args: [dep,al,col]},
						success: function(response) 
						{
						    if (response === "1") 
						    {
						    	errmsg = "This Department Name already exists."
						    	document.getElementById("dep-error").innerHTML=errmsg;
								$(".college-form-name").addClass("error-input");
						    }
						    else if (response === "0")
						    {
						    	...
						    	window.location = "ddepartment.php";
			...
			<div id='college-view-body' class="content-body">
				...
				<div class="content-line"></div>
				<?php if (!empty($_SESSION['added_dep'])): 
				...
				?>
					...
					<div id="success-box" class="success-box-acc"><span class='fa fa-times-circle success-close' 
						onclick='success_hide()'></span><?php echo $_SESSION['added_dep']; ?> has been successfully added.</div>

					<?php $_SESSION['added_dep'] = null ?>
				<?php endif ?>
				<table id='tbl' class='content-table'>
					...
					<tr>
					<?php if ($x%2 == 0): ?>
						<td class="row-2"><?php echo $department["name"] . " " . "[" . $department["alias"] . "]"; ?></td>
						<td class="row-2"><?php echo $department["college"]; ?></td>
						<td class="row-2"><?php echo $department["date"]; ?></td>
						<td class="row-2">
							<input class="tbl-edit" type="submit" value="Edit">
							<input class="tbl-del" type="submit" value="Delete">
						</td>
					<?php else: ?>
						<td><?php echo $department["name"] . " " . "[" . $department["alias"] . "]"; ?></td>
						<td><?php echo $department["college"]; ?></td>
						<td><?php echo $department["date"]; ?></td>
						<td>
							<input class="tbl-edit" type="submit" value="Edit">
							<input class="tbl-del" type="submit" value="Delete">
						</td>
